...
//! Operates over byte slices using only fixed-size stack buffers (no `alloc`).
...
pub fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    tail[rem_len] = 0x80;
    let tail_len = if rem_len + 1 + 8 <= 64 { 64 } else { 128 };
    tail[tail_len - 8..tail_len].copy_from_slice(&bit_len.to_be_bytes());

    process_block(&mut h, &tail[..64]);
    if tail_len == 128 {
        process_block(&mut h, &tail[64..128]);
    }
